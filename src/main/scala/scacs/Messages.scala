 ...
 * @param numBuffers The number of one-place buffers each {{ClusterService}} has access to (same for all nodes).
 */
case class InitializeClusterService(addresses: List[(String, Int)], numBuffers: Int)
 ...
 * @param fun The function to distribute.
 * @param input The input data to distribute and process.
 * @param trackingNumber The tracking number generated by {{MasterService}} to be used by {{ClusterService}} to locally store result. This is an optional. If no trackingNumber is provided, then the result is not stored locally by {{ClusterService}}.
 */
